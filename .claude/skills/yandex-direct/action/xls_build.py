...
COLUMNS = [
    ...
    ("Заголовки быстрых ссылок", 40), ("Описания быстрых ссылок", 46),
    ("Адреса быстрых ссылок", 46),
...
def rows_for_group(camp, group, gnum):
    ...
    for i, ad in enumerate(ads):
        ...
        sl = ad.get("sitelinks", [])[:8]          # справка: до восьми быстрых ссылок
        row = {
            ...
            # в XLS быстрые ссылки лежат в трёх колонках, элементы разделяются «||»
            "Заголовки быстрых ссылок": "||".join(s.get("title", "") for s in sl),
            "Описания быстрых ссылок": "||".join(s.get("description", "") for s in sl)
                                       if any(s.get("description") for s in sl) else "",
            "Адреса быстрых ссылок": "||".join(s.get("href", "") for s in sl),
